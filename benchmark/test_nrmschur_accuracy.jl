...
data = zeros(5, 3)
N = 100
@views(for (i, n) ∈ enumerate([10, 33, 100, 333, 1000])
    global data
    for k ∈ 1:N
        #Q = create_Q(rand(Uniform(π/2 - e, π/2 + e), n ÷ 2), isodd(n))  #Worst accuracy scenario
        Q = create_Q(rand(Uniform(0, π), n ÷ 2), isodd(n))
        T, V = nrmschur(Q)
        S = schur(Q)
